...
bool EpubReaderActivity::updateReaderState() {
  ...
  // The toolbar owns input while visible. Keeping this in the EPUB-specific
  // state hook leaves all ordinary reader controls in ReaderActivity::loop().
...
bool EpubReaderActivity::performReaderAction(const ReaderAction action, const int x, const int y) {
  switch (action) {
    ...
    case ReaderAction::ActivateAtPoint: {
      if (!section || !epub || currentPageLinks.empty() || isAtEndOfBook()) return false;
      const auto* link = EpubReaderUtils::linkAtPoint(currentPageLinks, x, y, currentPageLinkMarginLeft,
    ...
    case ReaderAction::OpenLinks:
      if (linkDepth > 0) {
        ...
        auto linksActivity = makeUniqueNoThrow<EpubReaderLinksActivity>(renderer, mappedInput, currentPageLinks);
        if (!linksActivity) {
...
void EpubReaderActivity::buildMoreActions() {
  ...
  EpubReaderMenuActivity::buildMenuItems(moreItems, !currentPageLinks.empty(), !cachedBookmarks.empty());
...
void EpubReaderActivity::navigateToHref(const std::string& hrefStr, const bool savePosition) {
  ...
  if (savePosition && section && linkDepth < MAX_LINK_DEPTH) {
    savedPositions[linkDepth] = {currentSpineIndex, section->currentPage};
    linkDepth++;
    LOG_DBG("ERS", "Saved position [%d]: spine %d, page %d", linkDepth, currentSpineIndex, section->currentPage);
  }
  ...
  if (targetSpineIndex < 0) {
    ...
    if (savePosition && linkDepth > 0) linkDepth--;
    return;
  }
...
void EpubReaderActivity::restoreSavedPosition() {
  ...
  linkDepth--;
  const auto& pos = savedPositions[linkDepth];
  LOG_DBG("ERS", "Restoring position [%d]: spine %d, page %d", linkDepth, pos.spineIndex, pos.pageNumber);
